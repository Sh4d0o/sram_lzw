...
int main(int argc, char *argv[]) {
    ...
    int forceflag = 0; // if true rle is always worth
    ...
    while ((opt = getopt(argc, argv, "dfs:")) != -1) {
        ...
        case 'f':
            forceflag = 1;
            pdebug(debugflag, "Force enconding active.\n");
            break;
        case 's':
            sizeflag = 1;
    ...
    char *dest_path = my_concat("./compressed/", compress_name);
    ...
    size_t nbytes = 0; // quantity of bytes read per block
    ...
    // 5.1.read 1st block
    while (nbytes != 0) {
        nbytes = fread(buffer_in, 1, block_size, src_file);
        // 5.2.process block
